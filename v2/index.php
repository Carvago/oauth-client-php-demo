...
if (str_contains($_SERVER['REQUEST_URI'], '/refresh-token')) {
    if (isset($_SESSION['refresh_token'])) {
        ...
        curl_setopt($ch, CURLOPT_URL, $authServerUrl . '/admin/v2/oauth/refresh-token');
...
if (str_contains($_SERVER['REQUEST_URI'], '/' . $callbackPath)) {
    if (isset($_GET['code'], $_GET['state'])) {
        ...
        $ch = curl_init();
        // POST request to obtain access token
        curl_setopt($ch, CURLOPT_URL, $authServerUrl . '/admin/v2/oauth/access-token');
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_HTTPHEADER, [
            'Accept: application/json',
